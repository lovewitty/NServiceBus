namespace NServiceBus
{
    ...
    class ReceiveWithTransactionScope : ReceiveStrategy
    {
        ...
        async Task<bool> ProcessMessage(Message message, Dictionary<string, string> headers)
        {
            ...
            {
                var shouldRetryImmediately = await HandleError(message, headers, failureInfo.Exception, transportTransaction, failureInfo.NumberOfProcessingAttempts).ConfigureAwait(false);
